...
const getSearch=((req,res)=>{
    ...
    try{
            ...
            var data=async()=>
            {
                if(onlyid!=undefined)
                {
                    var result=await db.fatchdata(`select * from student_master where student_master.std_id=${onlyid}`);
                }
                else if(op=='and' && std_id!="" && first_name!="" && last_name!="" && contact!="" && email!="" && gender!="")
                {
                    var result=await db.fatchdata(`select * from student_master where student_master.std_id=${std_id} ${op} first_name='${first_name}' ${op} last_name='${last_name}' ${op} contact='${contact}' ${op} email='${email}' ${op} gender='${gender}'`);
                }
                else if(op=='or')
                {
                    var result=await db.fatchdata(`select * from student_master where std_id="${std_id}" ${op} first_name="${first_name}" ${op} last_name="${last_name}" ${op} contact="${contact}" ${op} email="${email}" ${op} gender="${gender}";`)
                }
